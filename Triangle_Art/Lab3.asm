...
Main:
	li t1, 4	#row (bc rows 1-3 do not go through loop)
	li t3, 0	#blank_spaces
	li t2, 2
	li t4, 0	
	...
	bgtz t0, FirstTwoRows			# if t0 is > 0, we skip the InvalidInput thingy
...
FirstTwoRows:
	...
	blt t0, t2, exit_branch 	# if t0 is less than 2 (it is 1) branch to exit
	...
	addi t2, t2, 1
	blt t0, t2, exit_branch 	# if t0 is less than 3 (it is 2) branch to exit
	
	addi t2, t2, 1
	blt t0, t2, last_row		# if t0 is less than 4 (it is 3) branch to print last line. write that first
	
#t0 has to be at least 4
#so, we want to loop the blank spaces for 4-3
loop: 
	bgt t1, t0, last_row #rows <= (inp) stay in loop. so rows > (inp) , branch: last_row (exit loop)
	print_str(star)
	write_to_buffer(0x2a)
	addi t3, t1, -2		#t3 is 2
	

blank_space_loop:
	#blez t3, loop_update		#if t3 (blank spaces) is less than or equal to zero, exit 
	li t2, 1			
	ble t3, t2, loop_update		#idk why, but it prints the correct number of blank spaces when we do t3 < 1, branch
	print_str(blackspace)
	write_to_buffer(0x20)

blank_space_loop_update:
	addi t3, t3, -1
	b blank_space_loop
